...
function useConvertRoute(
    ...
    customDirectory: string = "app",
    customEntryPage: string = "page"
) {
     ...
     */
    function getLayoutForPath(path: string): React.ComponentType<any> | undefined {
 ...
 */
function LayoutWrapper({ Layout, Page }: { Layout: React.ComponentType<any>, Page: React.ComponentType<any> }) {
    return Layout ? (
        <Layout>
            <Page />
        </Layout>
 ...
 */
function useGetPages(typeOfImport: "vite" | "webpack"): Record<string, any> {
    if (typeOfImport === "vite") {
        if (typeof import.meta !== "undefined") {
            // console.log("Using import.meta");
            const pages = (import.meta as any).glob("./app/**/!(*.test.[jt]sx)*.([jt]s|[jt]sx)", {
                eager: true,
            });
            return pages;
        }
    ...
    if (typeOfImport === "webpack") {
        ...
        const ctx = (require as any).context('./app', true, /^(?!.*\.test\.[jt]sx?$).*\.[jt]sx?$/);
...
const RouteParamContext = createContext<string | undefined>(undefined);
 ...
 */
export default function FileBasedRoutes({ typeOfImport }: { typeOfImport: "vite" | "webpack" }) {
